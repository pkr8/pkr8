...
<body style="background: no-repeat center center fixed; background-color: black; background-image:url('photos/court.png'); background-size: cover; overflow-x: hidden; padding: 0px; height: 100%; margin: 0px; width: 100%;">
    <?php
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST"){
                if  (!isset($_POST['username']) && !isset($_POST['password2']) && !isset($_POST['password'])) {
                        die();
                }
        session_start();
	...
	if ($password2 != $mypassword){
	   $passwordErr="The passwords are not the same";
	}
	else{
	try{
        ...
        try{
                ...
                $st =$dbh->prepare("INSERT INTO DukeUser VALUES(?,CAST(? as TIMESTAMP), CAST (? as TIMESTAMP),?)");
                $st->execute(array($_SESSION['username'], date('m-d-y',time()), null, $mypassword ));
...
#E5E5E5; opacity: .8; index: 5;  width: 60%;">
            ...
            <div>
                <h1 id="errorMessage" style="color: black; font-family: 'Segoe UI light'">Register</h1>
            </div>
            <table style="width: 100%; background-color: #666666;  margin-top: 40px">
             ...
             <table style="width: 100%; min-width: 600px;  margin-top: 50px;" >
                 <tr>
                     <td valign="top" align="left" style="width: 50%; padding-left: 10%; ">
                         <h2 style="color:#001A57; font-size: 20px; font-family: 'Segoe Ui'; padding-bottom: 10px;">Not Registered? Sign Up Today!</h2>
			 <p style="color: red">* required fields</p> 
                         <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" method="post" role="form" style="padding-bottom: 100px">
			     ...
			     <span style="color: red">* <?php echo $passwordErr;?></span>      
                             <h2 style="color: black; font-size: 16px; font-family: 'Segoe Ui' ; ">Re-type password: </h2>
                             <input type="password" name="password2" required/>
			     <span style="color: red">*</span>   
                             <h2></h2>
                             <input style="color: black; background-color: #666666; border-style: none; padding: 5px; padding-right: 10px; padding-left: 10px; font-family:'Segoe UI'; color: white" type="submit" name="submit" value="Submit" />

                         </form>
                     </td>
		     <td valign="top" align="center"; style="padding-top: 100px; padding-right: 10%; height: 100%; width: 50%">
