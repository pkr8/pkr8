...
<body style="background: no-repeat center center fixed; background-color: black; background-image:url('photos/court.png'); background-size: cover; overflow-x: hidden; padding: 0px; height: 100%; margin: 0px; width: 100%;">
    ...
    $mylog = "scrapeLog.txt";
    $logs = file($mylog);
    $lastlog = (int) $logs[count($logs)-1];
    $isLog='false';
    if (time() - $lastlog >= 86400) {
	exec('python ../db-events/eventscraper.py', $output,$return);
	if ($return != 0){
		echo "python exec did not work";
	}
	$towrite = fopen('scrapeLog.txt', 'w');
	$txt = time();
	echo time()-$lastlog;
	fwrite($towrite,$txt);
	fclose($towrite);
   ...
   <div align="center" style="position: absolute;  top: 0px; width: 100%; ">
        <nav class="navbar navbar-default" style="background-color: #001A57; width: 100%; z-index: 1; opacity: .7; box-shadow: 5px 5px 20px 5px #303030; border-style: none">
            <div class="container-fluid">
			    ...
			    <li><a style="color:white" href="">Day View</a><li>
...
<script>
    ...
    if (<?php echo $isLog;?> == 'true'){
       xmlhttp=new XMLHttpRequest();
       url="deletePastEvents.php?mydate="+date.toISOString();
    ...
    function dateChange(){
        ...
        if ($("#group").is(":checked")){
           ...
           if (group==null){
               url2 = "changeDate.php?mydate="+date.toISOString();
           }
           else{
           	url2 = "changeGroup.php?mygroup="+group+"&mydate="+date.toISOString();
           }
	}
        else{
           var loc=$("#groups").val();
           if (loc==null){
              url2 = "changeDate.php?mydate="+date.toISOString();
           	...
           	url2 = "changeLocation.php?mylocation="+loc+"&mydate="+date.toISOString();
           }
        }
    ...
    function changeGroup(group){
        ...
        if ($("#group").is(":checked")){
           url2 = "changeGroup.php?mygroup="+group+"&mydate="+tempdate.toISOString();
	}
        else{
           url2 = "changeLocation.php?mylocation="+group+"&mydate="+tempdate.toISOString();
        }
